...
#+begin_src shell
  sudo -H nix --experimental-features "nix-command flakes" run \
...
#+begin_src shell
  ...
  ulimit -n 65536
  sudo -H nixos-install --flake .#X13Gen2 --no-root-passwd --cores 1 --max-jobs 1
#+end_src
...
- On each boot the ~@root~ subvolume is deleted and recreated blank ([[file:../hosts/X13Gen2/impermanence.nix][hosts/X13Gen2/impermanence.nix]]); ~@persist~, ~@home~, and ~@nix~ survive
