...
- 具体的な指示は次のコメントで与えられる
- 初期応答（定型文）: 「内容を理解しました。何を作りたい？」と返答すること
...
- **回答例**: 想定される回答パターンと、それぞれが設計に与える影響※必ず回答例の推奨方針には（推奨）と明記すること
...
プロジェクトの適切なフォルダ配下にMarkdownで作成した要件定義書を配置すること。そのファイル名はユーザーへ通知すること
また、要件定義所の中は適切なタイミングでmermaidを利用すること。
