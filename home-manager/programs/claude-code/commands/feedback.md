...
1. **会話履歴からの抽出**: セッション内でClaude Codeが使用したEdit/Writeツールの対象ファイルを特定
2. **フォールバック（git status）**: 会話履歴から特定できない場合、git statusで未コミット変更を検出
3. **ユーザー確認**: それでも不明な場合、ユーザーに対象を確認

**会話履歴からの抽出方法**:

```
- Edit/Writeツールの呼び出し履歴を確認
- 対象ファイルパスをリストアップ
- 重複を除去して対象ファイル一覧を作成
```
...
```markdown
レビュー対象を特定できませんでした。レビュー対象のファイルパスを指定してください。
```
