...
residue="$(python3 - "$skills_dir" "$prompts_dir" <<'PY'
...
files.each do |f|
  ...
  yaml_mode="python"
  yaml_out="$(python3 - "$skills_dir" <<'PY'
...
This check fails closed on purpose: the list is confidential and is never committed here."
  ...
  leak="$(python3 - "$denylist_file" "$ai_tools_dir" <<'PY'
...
ref_out="$(python3 - "$prompts_dir" "$skills_dir" <<'PY'
