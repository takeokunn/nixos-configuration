#!/bin/bash
# PreToolUse:Bash hook that blocks a standalone `cd`, which has no effect since shell state
...
# if, not `[[ ... ]] && exit 0`, since that form returns 1 on the common path and set -e would abort.
