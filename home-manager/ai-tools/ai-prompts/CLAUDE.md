...
agent per remaining file. Write the partition down before writing the prompts — one held only in your head
...
agents to keep scratch files inside their own worktree — a fixed path outside the repository collides silently.
...
score, percent complete, dev-hour figure — none has a derivation; state the observable condition. A dismissal
...
untouched baseline is not a regression gate — record the baseline first. When failures arrive in a batch,
...
concrete evidence and still disagree, they are answering different questions or one read stale state — re-read
...
which earlier claim it negates — appending turns a statement of what is true into a changelog whose stale first
paragraph is what the next reader loads.
...
than asking about it — ask only when two readings produce different work and the wrong choice is expensive to
