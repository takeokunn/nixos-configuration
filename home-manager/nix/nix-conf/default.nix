{ config, ... }:
{
  ...
  # substituting. Keep every key here — and in any file included below — under
  # extra- so it only ever adds to, never replaces, the system-level
  # substituters.
  #
  ...
  # Work-specific substituters live outside this public repo: a private binary
  # cache's hostname names the employer. `!include` (unlike plain `include`)
  # is skipped without error when the file is absent, which keeps this
  # evaluating on a machine that has no work cache — and, more importantly,
  # keeps the generated nix.conf passing the `nix config show` checkPhase,
  # which runs in a sandbox where the path never exists.
  #
  # The extra- discipline described above applies inside that file too, and is
  # not enforceable from here — nothing in this repository can see its contents.
  nix.extraOptions = ''
