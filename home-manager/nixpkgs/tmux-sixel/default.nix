{ lib, stdenv, fetchFromGitHub, autoreconfHook, bison, libevent, ncurses
, pkg-config, libsixel, runCommand
...
in stdenv.mkDerivation (finalAttrs: {
  ...
  buildInputs = [ ncurses libevent libsixel ]
    ++ lib.optionals withSystemd [ systemd ]
    ++ lib.optionals withUtf8proc [ utf8proc ]
    ++ lib.optionals withUtempter [ libutempter ];

  configureFlags =
    [ "--sysconfdir=/etc" "--localstatedir=/var" "--enable-sixel" ]
