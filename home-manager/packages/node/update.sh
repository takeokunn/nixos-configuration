...
get_npm_packages() {
    ...
    # Excludes copilot-language-server (handled separately)
    perl -0777 -ne '
        while (/mkNpmPackage\s*\{(.*?)\};/gs) {
            ...
            $npm_name //= $pname;
            next if $pname eq "copilot-language-server";
            print "$pname\t$npm_name\n";
        }
    ' "$DEFAULT_NIX"
}

# Extract copilot platform list from default.nix
get_copilot_platforms() {
...
update_copilot_platform() {
    local platform="$1"
    local npm_name="@github/copilot-language-server-$platform"
    local latest_version="$2"
    local url new_hash new_sri

    echo "  Updating $npm_name hash..."

    # Get new hash
    url="https://registry.npmjs.org/@github/copilot-language-server-$platform/-/copilot-language-server-$platform-$latest_version.tgz"
    new_hash=$(nix-prefetch-url --unpack "$url" 2>/dev/null | tail -1)
    new_sri=$(nix hash convert --hash-algo sha256 --to sri "$new_hash")

    # Update hash for this platform
    perl -0777 -pi -e "s/(platform = \"$platform\".*?hash = \")sha256-[^\"]+/\${1}$new_sri/s" "$DEFAULT_NIX"
}

# Update copilotVersion first (shared across all copilot packages)
echo "Checking copilot-language-server version..."
copilot_current=$(perl -ne 'print $1 if /copilotVersion = "([^"]+)/' "$DEFAULT_NIX" | head -1)
copilot_latest=$(npm view "@github/copilot-language-server" version 2>/dev/null || echo "")

if [[ -n "$copilot_latest" && "$copilot_current" != "$copilot_latest" ]]; then
    echo "  Updating copilotVersion from $copilot_current to $copilot_latest"
    perl -pi -e "s/copilotVersion = \"$copilot_current\"/copilotVersion = \"$copilot_latest\"/" "$DEFAULT_NIX"

    # Update main copilot-language-server hash
    url="https://registry.npmjs.org/@github/copilot-language-server/-/copilot-language-server-$copilot_latest.tgz"
    new_hash=$(nix-prefetch-url --unpack "$url" 2>/dev/null | tail -1)
    new_sri=$(nix hash convert --hash-algo sha256 --to sri "$new_hash")
    perl -0777 -pi -e "s/(pname = \"copilot-language-server\".*?hash = \")sha256-[^\"]+/\${1}$new_sri/s" "$DEFAULT_NIX"

    # Update package-lock.json
    lock_file="copilot-language-server/package-lock.json"
    if [[ -f "$lock_file" ]]; then
