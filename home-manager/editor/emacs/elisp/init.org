...
#+begin_src emacs-lisp :tangle yes
  ...
  ;; corfu child-frames require a GUI frame; defer in daemon mode
  (defun my/setup-corfu-daemon ()
    (global-corfu-mode 1)
    (remove-hook 'server-after-make-frame-hook #'my/setup-corfu-daemon))

  (if (daemonp)
      (add-hook 'server-after-make-frame-hook #'my/setup-corfu-daemon)
