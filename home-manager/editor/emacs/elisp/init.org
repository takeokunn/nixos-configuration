...
\(tolerating failure), refs/remotes/origin/HEAD, origin/main, then local HEAD."
  ...
  (defun my/ghq--link-state (repo-path target-path)
    "Symlink REPO-PATH's shared .state/.serena/memories and .state/.claude into
TARGET-PATH, mirroring __fzf_ghq_new_worktree.fish. Skips (with a message)
any target the checkout already provides as a real path or existing symlink,
...
\(file-exists-p alone would miss a broken symlink; file-symlink-p covers it)."
  ...
  (defun my/ghq-create-worktree (repo-path &optional base-ref)
...
`my/ghq--resolve-base-ref'. Returns the created worktree's absolute path."
    (let* ((base-ref (or base-ref (my/ghq--resolve-base-ref repo-path)))
           ...
           (base-name (format "%s-%s" (format-time-string "%Y%m%dT%H%M%S") base-sha))
            ...
            (if (my/ghq-bare-repository-p repo)
                (let ((worktrees (my/ghq-repo-worktrees repo)))
                  (if (null worktrees)
                    ...
                    (let ((choice (completing-read "Worktree: " (cons "[new worktree]" worktrees) nil t)))
