...
#+begin_src emacs-lisp :tangle yes
  ...
  (add-hook 'after-init-hook #'my/update-projectile-known-projects)
...
checked out normally contributes its own path, plus any worktrees it carries."
    (let (roots)
      (dolist (repo (split-string (shell-command-to-string "ghq list --full-path") "\n" t))
...
#+begin_src emacs-lisp :tangle yes
  ...
  (with-eval-after-load 'copilot-chat
    ...
    (setopt copilot-chat-default-model "claude-haiku-4.5"))
