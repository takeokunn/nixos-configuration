...
* Macro Utilities
:PROPERTIES:
:CUSTOM_ID: macro-utilities
:END:
** when-darwin
:PROPERTIES:
:CUSTOM_ID: when-darwin
:END:
#+begin_src emacs-lisp :tangle yes
...
:PROPERTIES:
:CUSTOM_ID: when-darwin-not-window-system
:END:
#+begin_src emacs-lisp :tangle yes
...
#+begin_src emacs-lisp :tangle yes
  (defmacro when-linux (&rest body)
    (when (string= system-type "gnu/linux")
      `(progn ,@body)))
#+end_src
** when-linux-not-window-system
:PROPERTIES:
:CUSTOM_ID: when-linux-not-window-system
:END:
#+begin_src emacs-lisp :tangle yes
  (defmacro when-linux-not-window-system (&rest body)
...
:PROPERTIES:
:CUSTOM_ID: temporarily-disable-magic-file-name
:END:
#+begin_src emacs-lisp :tangle yes
...
:END:
#+begin_src emacs-lisp :tangle yes
  ;; compat-30 appears before compat-31 in load-path; load compat-31 explicitly
  ;; so that Emacs 31 compatibility functions (e.g. set-local used by corfu) are available
...
** 末尾のスペースを可視化する
:PROPERTIES:
:CUSTOM_ID: visualize-trailing-spaces
:END:
#+begin_src emacs-lisp :tangle yes
...
** カッコが自動的に作られるようにする
:PROPERTIES:
:CUSTOM_ID: auto-insert-parentheses
:END:
#+begin_src emacs-lisp :tangle yes
...
:PROPERTIES:
:CUSTOM_ID: global-auto-revert-mode
:END:
#+begin_src emacs-lisp :tangle yes
...
** osx-clipboard
:PROPERTIES:
:CUSTOM_ID: osx-clipboard
:END:
#+begin_src emacs-lisp :tangle yes
  (autoload-if-found '(osx-clipboard-mode) "osx-clipboard" nil t)
...
:END:
#+begin_src emacs-lisp :tangle yes
  (autoload-if-found '(global-clipetty-mode) "clipetty" nil t)
...
:PROPERTIES:
:CUSTOM_ID: terminal-escape-sequence-handling
:END:
#+begin_src emacs-lisp :tangle yes
  ;; Terminal escape sequence handling for daemon compatibility
  ;; When running as daemon, (display-graphic-p) is evaluated at daemon start,
  ;; not per-frame. Use after-make-frame-functions to handle terminal clients.

  (defun my/setup-terminal-escape-sequences (&optional frame)
    "Set up terminal-specific keybindings for FRAME.
Handles focus reporting (DECSET 1004) and bracketed paste (DECSET 2004)."
    (with-selected-frame (or frame (selected-frame))
      (unless (display-graphic-p frame)
        ;; Focus reporting - prevents 'OI' characters
        (define-key input-decode-map "\e[I" [focus-in])
        (define-key input-decode-map "\e[O" [focus-out])
        (define-key special-event-map [focus-in] #'ignore)
        (define-key special-event-map [focus-out] #'ignore)
        ;; Bracketed paste - prevents '^[[200~' and '^[[201~' characters
        ;; Use xterm-translate-bracketed-paste to properly create event with pasted text
        (require 'term/xterm)
        (define-key input-decode-map "\e[200~" #'xterm-translate-bracketed-paste))))

  ;; Apply to new frames (handles emacsclient connections to daemon)
  (add-hook 'after-make-frame-functions #'my/setup-terminal-escape-sequences)

  ;; Apply to current frame if not running as daemon
  (unless (daemonp)
    (my/setup-terminal-escape-sequences))
#+end_src
** native comp
:PROPERTIES:
:CUSTOM_ID: native-comp
:END:
#+begin_src emacs-lisp :tangle yes
...
** 同一bufferの名前を変える
:PROPERTIES:
:CUSTOM_ID: uniquify-buffer-names
:END:
#+begin_src emacs-lisp :tangle yes
...
:PROPERTIES:
:CUSTOM_ID: enable-local-variables
:END:
#+begin_src emacs-lisp :tangle yes
...
** 検索で大文字小文字を区別しない
:PROPERTIES:
:CUSTOM_ID: case-insensitive-search
:END:
#+begin_src emacs-lisp :tangle yes
  (with-eval-after-load 'minibuffer
...
** kill-ringのサイズを変更
:PROPERTIES:
:CUSTOM_ID: change-kill-ring-size
:END:
#+begin_src emacs-lisp :tangle yes
...
* Global Keybind
:PROPERTIES:
:CUSTOM_ID: global-keybind
:END:
** basic
:PROPERTIES:
:CUSTOM_ID: global-keybind-basic
:END:
#+begin_src emacs-lisp :tangle yes
...
** direnv-mode
:PROPERTIES:
:CUSTOM_ID: direnv-mode
:END:
#+begin_src emacs-lisp :tangle yes
  (when (autoload-if-found '(direnv-envrc-mode direnv-mode direnv-allow) "direnv" nil t)
...
:PROPERTIES:
:CUSTOM_ID: todo-graphviz-dot-mode
:END:
#+begin_src emacs-lisp :tangle yes
...
:END:
#+begin_src emacs-lisp :tangle yes
  (autoload-if-found '(nskk-mode nskk-global-mode) "nskk" nil t)
  ...
  (with-eval-after-load 'nskk
    ...
    ;; cursor color (dracula palette)
    (setopt nskk-use-color-cursor t)
    (set-face-attribute 'nskk-cursor-hiragana nil :background "#50fa7b")
    (set-face-attribute 'nskk-cursor-katakana nil :background "#8be9fd")
    (set-face-attribute 'nskk-cursor-latin nil :background "#ff79c6")
...
#+begin_src emacs-lisp :tangle yes
  ...
  (defun my/update-magit-repository-directories (&optional roots)
    "Populate `magit-repository-directories' from the same ghq-aware root set
that backs `projectile-known-projects' (bare repos contribute only their
worktree children; non-bare repos contribute themselves), each paired with
depth 0 per Magit's (DIRECTORY . DEPTH) format (magit-repository-directories
docstring / magit.vc Repository List manual).  ROOTS, when supplied, is used
directly instead of recomputing via `my/ghq-project-roots'."
    (interactive)
    (setopt magit-repository-directories
            (mapcar (lambda (d) (cons (abbreviate-file-name (file-name-as-directory d)) 0))
                    (or roots (my/ghq-project-roots)))))
...
:PROPERTIES:
:CUSTOM_ID: git-auto-commit-mode
:END:
#+begin_src emacs-lisp :tangle yes
...
#+begin_src emacs-lisp :tangle yes
  ...
  (add-hook 'after-init-hook #'global-dmacro-mode)
#+end_src
*** god-mode
:PROPERTIES:
:CUSTOM_ID: god-mode
...
*** soft-narrow
:PROPERTIES:
:CUSTOM_ID: soft-narrow
:END:
#+begin_src emacs-lisp :tangle yes
  (autoload-if-found '(soft-narrow-mode) "soft-narrow" nil t)

  (add-hook 'after-init-hook #'soft-narrow-mode)
#+end_src
** LSP
:PROPERTIES:
:CUSTOM_ID: lsp
...
#+begin_src emacs-lisp :tangle yes
  ...
  (with-eval-after-load 'lsp-nix
    ...
    (let ((flake-path (concat "builtins.getFlake \""
                              (my/ghq-repo-path "github.com/takeokunn/nixos-configuration")
                              "\"")))
      (setopt lsp-nix-nixd-nixpkgs-expr
              (format "import (%s).inputs.nixpkgs { }" flake-path))
      (setopt lsp-nix-nixd-nixos-options-expr
              (format "(%s).darwinConfigurations.M4-Max.options" flake-path))
      (setopt lsp-nix-nixd-home-manager-options-expr
              (format "(%s).darwinConfigurations.M4-Max.options.home-manager.users.type.getSubOptions []" flake-path))))
...
#+begin_src emacs-lisp :tangle yes
  ...
  (add-hook 'after-init-hook #'puni-global-mode)
...
*** smart-jump
:PROPERTIES:
:CUSTOM_ID: smart-jump
:END:
#+begin_src emacs-lisp :tangle yes
  (autoload-if-found '(smart-jump-setup-default-registers) "smart-jump" nil t)

  (add-hook 'after-init-hook #'smart-jump-setup-default-registers)
#+end_src
*** string-inflection
:PROPERTIES:
...
:END:
#+begin_src emacs-lisp :tangle yes
  (autoload-if-found '(wdired-change-to-wdired-mode) "wdired" nil t)
...
:PROPERTIES:
:CUSTOM_ID: auto-save-buffers-enhanced
:END:
#+begin_src emacs-lisp :tangle yes
  (autoload-if-found '(auto-save-buffers-enhanced) "auto-save-buffers-enhanced" nil t)

  ;; (add-hook 'after-init-hook #'auto-save-buffers-enhanced)
...
#+begin_src emacs-lisp :tangle yes
  ...
  (with-eval-after-load 'recentf
    ...
    (setopt recentf-exclude
            (list (regexp-quote (expand-file-name ".recentf" user-emacs-directory))
                  "\\.gpg\\'")))
#+END_SRC
** open-junk-file
:PROPERTIES:
:CUSTOM_ID: open-junk-file
:END:
#+begin_src emacs-lisp :tangle yes
...
#+begin_src emacs-lisp :tangle yes
  ...
  (add-hook 'after-init-hook #'my/update-ghq-project-state)

  (defun my/ghq-bare-repository-p (dir)
    "Return non-nil when DIR is a bare repository.
Tests for the absence of a .git entry rather than a .git suffix on the
directory name, so the answer does not depend on ghq's naming convention."
    (not (file-exists-p (expand-file-name ".git" dir))))

  (defun my/ghq-repo-path (relative)
    "Return a directory holding the working files of RELATIVE.
RELATIVE is a ghq-style path such as \"github.com/takeokunn/blog\".  Under the
bare layout the repository is <root>/RELATIVE.git and contains no file to
visit, so resolve instead to one of its worktrees: the fixed `main' one, or
failing that the newest timestamped one — those names sort chronologically.
A repository still checked out normally resolves to its own path, so this is
safe to use for every repository regardless of which layout it is in."
    (let* ((root (string-trim (shell-command-to-string "ghq root")))
           (plain (expand-file-name relative root))
           (worktrees (expand-file-name ".worktrees" (concat plain ".git")))
           (main (expand-file-name "main" worktrees)))
      (cond
       ((file-directory-p plain) plain)
       ((file-directory-p main) main)
       ((file-directory-p worktrees)
        (let ((dirs (seq-filter #'file-directory-p
                                (directory-files worktrees t directory-files-no-dot-files-regexp))))
          (if dirs (car (last dirs)) plain)))
       (t plain))))

  (defun my/ghq-project-roots ()
    "Return the directories projectile should treat as projects.
`ghq list --full-path' reports one path per repository, but under the bare
layout that path is <repo>.git, which holds no working tree and therefore no
file to visit.  What is navigable there are its worktrees, so a bare entry
contributes its .worktrees/* children and nothing else.  A repository still
checked out normally contributes its own path, plus any worktrees it carries."
    (let (roots)
      (dolist (repo (my/ghq-repo-list))
        (unless (my/ghq-bare-repository-p repo)
          (push repo roots))
        (let ((worktrees (expand-file-name ".worktrees" repo)))
          (when (file-directory-p worktrees)
            (dolist (dir (directory-files worktrees t directory-files-no-dot-files-regexp))
              (when (file-directory-p dir)
                (push dir roots))))))
      (nreverse roots)))

  (defun my/ghq-repo-list ()
    "Return the list of ghq-managed repository paths (bare and non-bare)."
    (split-string (shell-command-to-string "ghq list --full-path") "\n" t))

  (defun my/ghq--git (repo-path &rest args)
    "Run git ARGS in REPO-PATH; return trimmed stdout, or nil on nonzero exit.
Uses `call-process' with an argument list rather than a shell command string,
so a value in ARGS or REPO-PATH can never be reinterpreted as shell syntax."
    (with-temp-buffer
      (let ((exit (apply #'call-process "git" nil (list t nil) nil "-C" repo-path args)))
        (when (zerop exit)
          (string-trim (buffer-string))))))

  (defun my/ghq-repo-worktrees (repo-path)
    "Return REPO-PATH's existing worktree paths, excluding its own bare
git-common-dir entry."
    (let ((common-dir (my/ghq--git repo-path "rev-parse" "--path-format=absolute" "--git-common-dir"))
          (worktrees nil))
      (dolist (line (split-string (or (my/ghq--git repo-path "worktree" "list" "--porcelain") "") "\n" t))
        (when (string-prefix-p "worktree " line)
          (let ((wt (substring line (length "worktree "))))
            (unless (string= wt common-dir)
              (push wt worktrees)))))
      (nreverse worktrees)))

  (defun my/ghq--resolve-base-ref (repo-path)
    "Resolve a base ref in REPO-PATH via the same fallback chain as
__fzf_ghq_new_worktree.fish: an explicit-refspec fetch of origin/main
\(tolerating failure), refs/remotes/origin/HEAD, origin/main, then local HEAD."
    (when (my/ghq--git repo-path "remote" "get-url" "origin")
      (my/ghq--git repo-path "fetch" "origin" "+refs/heads/main:refs/remotes/origin/main"))
    (or (let ((ref (my/ghq--git repo-path "symbolic-ref" "refs/remotes/origin/HEAD")))
          (and ref (string-remove-prefix "refs/remotes/" ref)))
        (and (my/ghq--git repo-path "rev-parse" "--verify" "--quiet" "origin/main") "origin/main")
        (my/ghq--git repo-path "symbolic-ref" "--short" "HEAD")
        (error "my/ghq--resolve-base-ref: cannot resolve a base ref in %s" repo-path)))

  (defun my/ghq--link-state (repo-path target-path)
    "Link or copy REPO-PATH's shared .state/ contents into TARGET-PATH,
mirroring __fzf_ghq_new_worktree.fish. .state/.serena/memories and
.state/.claude are symlinked, so a memory written in one worktree stays
visible from every other worktree of the same repository. .state/.envrc and
.state/.env are instead copied: unlike the symlinked pair, per-worktree
environment settings are meant to diverge from that shared file
independently once created — a one-shot snapshot rather than something
every worktree must keep seeing live. Every target the checkout already
provides as a real path or existing symlink is skipped (with a message),
matching fish's `test -e \"$dst\"; or test -L \"$dst\"' shadow guard exactly
\(file-exists-p alone would miss a broken symlink; file-symlink-p covers it).
A missing .envrc/.env source is skipped silently, since most repositories
have neither. Immediately after a successful .envrc copy specifically (not
.env), runs `direnv allow' on TARGET-PATH when a `direnv' executable is
available; a failed or unavailable direnv is non-fatal, and a failure (as
opposed to unavailability) is reported via `message', mirroring fish's
stderr warning on the same failure."
    (let ((state-dir (expand-file-name ".state" repo-path)))
      (make-directory (expand-file-name ".serena/memories" state-dir) t)
      (make-directory (expand-file-name ".claude" state-dir) t)
      (dolist (pair '(".serena/memories" ".claude"))
        (let ((src (expand-file-name pair state-dir))
              (dst (expand-file-name pair target-path)))
          (if (or (file-exists-p dst) (file-symlink-p dst))
              (message "my/ghq--link-state: %s is provided by the checkout; leaving it unlinked" pair)
            (when (file-directory-p src)
              (make-directory (file-name-directory dst) t)
              (make-symbolic-link src dst)))))
      (dolist (name '(".envrc" ".env"))
        (let ((src (expand-file-name name state-dir))
              (dst (expand-file-name name target-path)))
          (cond
           ((or (file-exists-p dst) (file-symlink-p dst))
            (message "my/ghq--link-state: %s is provided by the checkout; leaving it uncopied" name))
           ((not (file-exists-p src)) nil)
           (t
            (copy-file src dst)
            (when (and (string= name ".envrc") (executable-find "direnv"))
              (condition-case err
                  (let ((direnv-exit (call-process "direnv" nil nil nil "allow" target-path)))
                    (unless (eq direnv-exit 0)
                      (message "my/ghq--link-state: direnv allow failed for %s (exit %s)" target-path direnv-exit)))
                (error
                 (message "my/ghq--link-state: direnv allow failed for %s (%s)" target-path (error-message-string err)))))))))))

  (defun my/ghq-create-worktree (repo-path &optional base-ref naming-mode)
    "Create a new worktree of the bare repository at REPO-PATH, Elisp-native
equivalent of __fzf_ghq_new_worktree.fish (fish's Serena serena_config.yml
projects: prune step is deliberately NOT reimplemented here — it needs YAML
parsing this Emacs config has no library for; that cleanup remains fish's
job for worktrees created there). BASE-REF, when omitted, is resolved via
`my/ghq--resolve-base-ref'. NAMING-MODE, when the symbol `branch', names the
created directory <timestamp>-<sanitized-base-ref> (leading \"origin/\"
stripped, remaining \"/\" replaced with \"-\") instead of the default
<timestamp>-<short-sha>, mirroring __fzf_ghq_new_worktree.fish's
naming_mode = branch case. Returns the created worktree's absolute path."
    (let* ((base-ref (or base-ref (my/ghq--resolve-base-ref repo-path)))
           (base-sha (or (my/ghq--git repo-path "rev-parse" "--short" base-ref)
                         (error "my/ghq-create-worktree: cannot resolve base ref %s in %s" base-ref repo-path)))
           (worktrees-dir (expand-file-name ".worktrees" repo-path))
           (base-name (if (eq naming-mode 'branch)
                          (format "%s-%s"
                                  (format-time-string "%Y%m%dT%H%M%S")
                                  (string-replace "/" "-" (string-remove-prefix "origin/" base-ref)))
                        (format "%s-%s" (format-time-string "%Y%m%dT%H%M%S") base-sha)))
           (target-name base-name)
           (suffix 2))
      (while (file-exists-p (expand-file-name target-name worktrees-dir))
        (setq target-name (format "%s-%d" base-name suffix))
        (setq suffix (1+ suffix)))
      (let ((target-path (expand-file-name target-name worktrees-dir)))
        (with-temp-buffer
          (unless (zerop (call-process "git" nil t nil "-C" repo-path "worktree" "add" "--detach" target-path base-sha))
            (error "my/ghq-create-worktree: failed to create worktree at %s: %s" target-path (buffer-string))))
        (my/ghq--link-state repo-path target-path)
        target-path)))

  (defun my/ghq--current-repo-path ()
    "Resolve the repository path containing `default-directory', mirroring
__fzf_ghq_new_worktree.fish's no-arg repo-path resolution: ask git for the
absolute git-common-dir and strip a trailing \"/.git\" suffix, if present, so
REPO-PATH names the bare repository itself rather than its .git contents."
    (let (common-dir)
      (with-temp-buffer
        (unless (zerop (call-process "git" nil t nil
                                      "rev-parse" "--path-format=absolute" "--git-common-dir"))
          (error "my/ghq--current-repo-path: not inside a git repository"))
        (setq common-dir (string-trim (buffer-string))))
      (string-remove-suffix "/.git" common-dir)))

  (defun my/ghq-create-worktree-default-branch ()
    "Create a new worktree of the current repository named after its resolved
default branch rather than a commit sha, then switch to it via Projectile.
Elisp equivalent of fish's `worktree_default_branch', which is exactly
`__fzf_ghq_new_worktree \"\" \"\" branch' — the same base-ref resolution as
the default flow, with only the created directory's naming changed. This is
deliberately a standalone command rather than an option inside
`my/ghq-switch-project's \"[new worktree]\" picker, mirroring fish's own
separate-command precedent: `worktree_default_branch' is not reachable from
`fzf_ghq's picker either."
    (interactive)
    (let* ((repo-path (my/ghq--current-repo-path))
           (target (my/ghq-create-worktree repo-path nil 'branch)))
      (projectile-switch-project-by-name target)))

  (defun my/ghq-switch-project ()
    "Two-stage project switch mirroring fish's fzf_ghq: pick a repository,
then (for a bare repository) pick an existing worktree or create a new one,
then hand off to Projectile's own switch machinery so
`projectile-switch-project-action' and existing hooks fire unchanged."
    (interactive)
    ;; The :before advice on `projectile-switch-project' can never reach this
    ;; code path (`:override' on the same symbol discards it, and this
    ;; function never calls `projectile-switch-project' anyway), so the
    ;; refresh has to happen explicitly here instead.
    (my/update-ghq-project-state)
    (let* ((repo (completing-read "Repository: " (my/ghq-repo-list) nil t))
           (target
            (if (my/ghq-bare-repository-p repo)
                (let ((worktrees (my/ghq-repo-worktrees repo)))
                  (if (null worktrees)
                      (my/ghq-create-worktree repo)
                    ;; Raw worktree paths are long and near-identical, so the
                    ;; picker shows each abbreviated the same way
                    ;; `my/update-projectile-known-projects' and
                    ;; `my/update-magit-repository-directories' already
                    ;; display project roots, while resolving the selection
                    ;; back to its real, non-abbreviated path via this alist.
                    (let* ((abbrev-alist (mapcar (lambda (w) (cons (abbreviate-file-name w) w)) worktrees))
                           (choice (completing-read "Worktree: "
                                                     (cons "[new worktree]" (mapcar #'car abbrev-alist))
                                                     nil t)))
                      (if (string= choice "[new worktree]")
                          (my/ghq-create-worktree repo)
                        (or (cdr (assoc choice abbrev-alist)) choice)))))
              repo)))
      (projectile-switch-project-by-name target)))

  (defun my/update-projectile-known-projects (&optional roots)
    "Rebuild `projectile-known-projects' from ROOTS, or from
`my/ghq-project-roots' if ROOTS is nil."
    (interactive)
    (projectile-clear-known-projects)
    (projectile-cleanup-known-projects)
    (setopt projectile-known-projects (mapcar
                                       (lambda (x)
                                         (abbreviate-file-name (file-name-as-directory x)))
                                       (or roots (my/ghq-project-roots)))))

  (defun my/update-ghq-project-state ()
    "Compute the ghq-aware project roots once per invocation and refresh both
Projectile's known-projects list and Magit's repository-directories from that
single computation, instead of each recomputing it independently."
    (interactive)
    (let ((roots (my/ghq-project-roots)))
      (my/update-projectile-known-projects roots)
...
#+begin_src emacs-lisp :tangle yes
  ...
  (with-eval-after-load 'projectile
    ...
    (advice-add 'projectile-switch-project :override #'my/ghq-switch-project)
...
:PROPERTIES:
:CUSTOM_ID: projectile-git-autofetch
:END:
#+begin_src emacs-lisp :tangle yes
  (autoload-if-found '(projectile-git-autofetch-setup) "projectile-git-autofetch" nil t)

  (add-hook 'after-init-hook #'projectile-git-autofetch-setup)
...
#+begin_src emacs-lisp :tangle yes
  ...
  (defun my/tab-bar-rename-tab (&rest _)
    (interactive)
    (let ((proj-name (if (and (fboundp 'projectile-project-p)
...
#+begin_src emacs-lisp :tangle yes
  ...
  (add-hook 'after-init-hook
            #'(lambda ()
                ...
                ;; Set default font size to 13pt (height is in 1/10pt units, match kitty)
...
:END:
#+begin_src emacs-lisp :tangle yes
  (autoload-if-found '(warm-mode) "warm-mode" nil t)
#+end_src
** Doom
:PROPERTIES:
:CUSTOM_ID: doom
  ...
  (add-hook 'after-init-hook
            #'(lambda ()
                (when (require 'doom-themes)
                  (let ((theme 'doom-dracula))
                    (load-theme theme t)
                    ;; Emacs 31's gnus.el makes `gnus-group-news-low' inherit
                    ;; from `gnus-group-news-low-empty', while doom-themes'
                    ;; own face table makes `gnus-group-news-low-empty'
                    ;; inherit from `gnus-group-news-low' -- a mutual
                    ;; face-inheritance cycle. This cycle is defined in
                    ;; doom-themes-base.el and affects every doom theme, not
                    ;; just this one -- `theme' is bound once above and
                    ;; reused below so switching themes cannot leave this
                    ;; fix only partially applied. gnus-group.el (which
                    ;; defines these faces) loads lazily, often long after
                    ;; this hook runs in a daemon session; the moment it
                    ;; does, Emacs resolves the cycle synchronously inside
                    ;; `custom-declare-face' and errors out before any
                    ;; `with-eval-after-load' hook keyed on `gnus-group'
                    ;; could ever run (verified: the require aborts before
                    ;; reaching `(provide 'gnus-group)'). So the theme's own
                    ;; registered face spec must be corrected here -- not
                    ;; the (not yet defined) face's attributes -- so the
                    ;; corrected, non-cyclic spec is already in place
                    ;; whenever gnus-group.el eventually defines the face.
                    (custom-theme-set-faces
                     theme '(gnus-group-news-low-empty ((t :inherit nil))))
                    (enable-theme theme)
...
#+begin_src emacs-lisp :tangle yes
  ...
  (add-hook 'after-init-hook #'(lambda () (line-number-mode 0)))
  (add-hook 'after-init-hook #'(lambda () (column-number-mode 0)))
  ...
  (with-eval-after-load 'doom-modeline
    ...
    ;; nskk segment
    ;; doom-modeline-def-segment is a macro: with-eval-after-load only defers
    ;; runtime execution, not macroexpansion at (native-comp) compile time, so
    ;; the macro must also be visible to the compiler here.
    (eval-when-compile (require 'doom-modeline))
    (doom-modeline-def-segment my/nskk
      "NSKK input mode indicator."
      (when (and (bound-and-true-p nskk-mode)
                 (fboundp 'nskk-modeline-indicator))
        (nskk-modeline-indicator)))
    (let ((main-segments
           (append '(compilation objed-state misc-info project-name persp-name battery grip irc)
                   (when (not (eq system-type 'darwin))
                     '(mu4e))
                   ;; Unlike the `mu4e' gate above (arm64 `mailutils' build
                   ;; failure, see commit b3ebd7e7), this avoids loading
                   ;; gnus-group.el on Darwin -- see the face-inheritance-cycle
                   ;; fix in the doom-themes block above, which is what this
                   ;; segment would otherwise trigger in a running daemon.
                   (when (not (eq system-type 'darwin))
                     '(gnus))
                   '(github debug repl lsp my/nskk minor-modes input-method indent-info buffer-encoding major-mode process vcs check time))))
      (doom-modeline-def-modeline 'main
        ...
        main-segments))
    ;; emacsclient -c でGUIフレームが生成されたとき modeline を再描画する
...
** idle-highlight-mode
:PROPERTIES:
:CUSTOM_ID: idle-highlight-mode-2
:END:
#+begin_src emacs-lisp :tangle yes
  (autoload-if-found '(idle-highlight-mode) "idle-highlight-mode" nil t)
...
:END:
#+begin_src emacs-lisp :tangle yes
  (autoload-if-found '(arto-open arto-version) "arto" nil t)

  (with-eval-after-load 'arto
    (setopt arto-path "/Users/take/Applications/Home Manager Apps/Arto.app"))
#+end_src
*** redacted
:PROPERTIES:
:CUSTOM_ID: redacted
...
#+begin_src emacs-lisp :tangle yes
  ...
  (add-to-list 'auto-mode-alist (cons (concat (regexp-quote (my/ghq-repo-path "github.com/takeokunn/private")) "/password-store/.*\\.gpg") 'pass-view-mode))
...
#+begin_src emacs-lisp :tangle yes
  ...
  ;; Configure variables BEFORE initialization
  (setopt exec-path-from-shell-arguments '("-l"))
  (setopt exec-path-from-shell-variables '("PATH" "TERM" "COPILOT_LANGUAGE_SERVER_PATH"))

  (exec-path-from-shell-initialize)

  ;; Explicitly set SSH_AUTH_SOCK for GPG agent (exec-path-from-shell doesn't handle this correctly on macOS)
...
#+begin_src emacs-lisp :tangle yes
  ...
  (add-hook 'after-init-hook
            (lambda ()
              (condition-case err
                  (pdf-tools-install t)
                (error (message "pdf-tools-install failed: %s" (error-message-string err))))))
#+end_src
** Jump
:PROPERTIES:
:CUSTOM_ID: jump
...
:END:
#+begin_src emacs-lisp :tangle yes
  (autoload-if-found '(doclive-preview-buffer
                       doclive-preview-file
                       doclive-stop-server
                       doclive-reload-page) "doclive" nil t)

  (with-eval-after-load 'markdown-mode
    (keymap-set markdown-mode-map "C-c M-v" #'doclive-preview-buffer))

  (with-eval-after-load 'org
    (keymap-set org-mode-map "C-c M-v" #'doclive-preview-buffer))
#+end_src
** Fish
:PROPERTIES:
:CUSTOM_ID: fish
...
:END:
#+begin_src emacs-lisp :tangle yes
  (autoload-if-found '(dasel-interactive) "dasel-interactive" nil t)
  (autoload-if-found '(dasel-convert) "dasel-convert" nil t)
  (autoload-if-found '(dasel-format-buffer dasel-format-region) "dasel-format" nil t)
  (autoload-if-found '(dasel-edit-put) "dasel-edit" nil t)
#+end_src
*** consult-dasel
:PROPERTIES:
:CUSTOM_ID: consult-dasel
:END:
#+begin_src emacs-lisp :tangle yes
  (autoload-if-found '(consult-dasel) "consult-dasel" nil t)
#+end_src
** Python
:PROPERTIES:
:CUSTOM_ID: python
...
#+begin_src emacs-lisp :tangle yes
  ...
  (with-eval-after-load 'elfeed-org
    ;; config
    (setopt rmh-elfeed-org-files (list (expand-file-name "project/elfeed.org" (my/ghq-repo-path "github.com/takeokunn/private"))))
    (setopt rmh-elfeed-org-auto-ignore-invalid-feeds t))
#+end_src
** elfeed-dashboard
:PROPERTIES:
:CUSTOM_ID: elfeed-dashboard
:END:
#+begin_src emacs-lisp :tangle yes
...
:END:
#+begin_src emacs-lisp :tangle yes
  (autoload-if-found '(kuro-create kuro-list-sessions kuro-attach kuro-kill) "kuro" nil t)

  (keymap-global-set "C-c t" #'kuro-create)
  (keymap-global-set "C-c T" #'kuro-list-sessions)
#+end_src
* Eshell
:PROPERTIES:
:CUSTOM_ID: eshell
...
:PROPERTIES:
:CUSTOM_ID: eshell-fringe-status
:END:
#+begin_src emacs-lisp :tangle yes
...
:PROPERTIES:
:CUSTOM_ID: eshell-syntax-highlight
:END:
#+begin_src emacs-lisp :tangle yes
...
:PROPERTIES:
:CUSTOM_ID: emacs-fish-completion
:END:
#+begin_src emacs-lisp :tangle yes
...
#+begin_src emacs-lisp :tangle yes
  ...
  (with-eval-after-load 'org
    ...
    ;; directory
    (setq org-directory (expand-file-name "agenda" (my/ghq-repo-path "github.com/takeokunn/private")))
  ...
  (with-eval-after-load 'org-capture
    ...
    (setq org-capture-templates `(("m" "Memo" entry (file ,(expand-file-name "memo.org" org-directory))
                                  ...
                                  ("j" "Journal" entry (file+olp+datetree ,(expand-file-name "journal.org" org-directory))
                                   "* %U\n%?\n%i\n")
                                  ("1" "Work" entry (file+headline ,(expand-file-name "task/work.org" org-directory) "Backlog")
                                   "** TODO Capture %U\n%?")
...
*** Basic
:PROPERTIES:
:CUSTOM_ID: org-mode-org-agenda-basic
:END:
#+begin_src emacs-lisp :tangle yes
...
** Org External Tools
:PROPERTIES:
:CUSTOM_ID: org-external-tools
:END:
*** org-redmine
:PROPERTIES:
:CUSTOM_ID: org-redmine
:END:
#+begin_src emacs-lisp :tangle yes
...
*** basic
:PROPERTIES:
:CUSTOM_ID: org-mode-org-babel-basic
:END:
#+begin_src emacs-lisp :tangle yes
...
:END:
#+begin_src emacs-lisp :tangle yes
  (autoload-if-found '(org-babel-execute:gleam) "ob-gleam" nil t)

  (with-eval-after-load 'org-src
    (add-to-list 'org-src-lang-modes '("gleam" . gleam)))
#+end_src
*** org-nix-shell
:PROPERTIES:
...
*** basic
:PROPERTIES:
:CUSTOM_ID: org-mode-org-roam-basic
:END:
#+begin_src emacs-lisp :tangle yes
  ...
  (with-eval-after-load 'org-roam
    ;; config
    (setopt org-roam-directory `,(my/ghq-repo-path "github.com/takeokunn/blog")))
#+end_src
*** org-roam-mode
:PROPERTIES:
:CUSTOM_ID: org-roam-mode
:END:
#+begin_src emacs-lisp :tangle yes
  ...
  (with-eval-after-load 'org-roam-capture
    ...
    (setopt org-roam-capture-templates '(("f" "Fleeting(一時メモ)" plain "%?"
                                         ...
                                         ("z" "Zenn" plain "%?"
                                          ...
                                          :unnarrowed t)
                                         ("t" "dev.to" plain "%?"
                                          :target (file+head "devto/articles/%<%Y%m%d%H%M%S>.org" "#+TITLE: ${title}\n")
                                          :unnarrowed t)
                                         ("m" "Private" plain "%?"
...
*** org-roam-ql-ql
:PROPERTIES:
:CUSTOM_ID: org-roam-ql-ql
:END:
#+begin_src emacs-lisp :tangle yes
  (autoload-if-found '(org-roam-ql-ql-init) "org-roam-ql-ql" nil t)

  (add-hook 'after-init-hook #'org-roam-ql-ql-init)
#+end_src
** Citar
:PROPERTIES:
:CUSTOM_ID: citar
...
*** citar
:PROPERTIES:
:CUSTOM_ID: citar-basic
:END:
#+begin_src emacs-lisp :tangle yes
  (autoload-if-found '(citar-open
                       citar-open-entry
                       citar-open-files
                       citar-open-notes
                       citar-insert-citation
                       citar-insert-reference
                       citar-insert-bibtex) "citar" nil t)

  (autoload-if-found '(citar-org-local-bib-files) "citar-org" nil t)

  (keymap-global-set "C-c b o" #'citar-open)
  (keymap-global-set "C-c b e" #'citar-open-entry)
  (keymap-global-set "C-c b f" #'citar-open-files)
  (keymap-global-set "C-c b n" #'citar-open-notes)
  (keymap-global-set "C-c b i" #'citar-insert-citation)

  (with-eval-after-load 'citar
    ;; config
    (setopt citar-bibliography
            `,(list (expand-file-name "hugo/references.bib" (my/ghq-repo-path "github.com/takeokunn/blog"))))

    ;; org-cite integration
    (setopt org-cite-insert-processor 'citar)
    (setopt org-cite-follow-processor 'citar)
    (setopt org-cite-activate-processor 'citar))
#+end_src
*** citar-embark
:PROPERTIES:
:CUSTOM_ID: citar-embark
:END:
#+begin_src emacs-lisp :tangle yes
  (autoload-if-found '(citar-embark-mode) "citar-embark" nil t)
...
*** citar-org-roam
:PROPERTIES:
:CUSTOM_ID: citar-org-roam
:END:
#+begin_src emacs-lisp :tangle yes
  (autoload-if-found '(citar-org-roam-mode
                       citar-org-roam-cited
                       citar-org-roam-ref-add
                       citar-org-roam-open-current-refs) "citar-org-roam" nil t)
  ...
  (with-eval-after-load 'citar-org-roam
    ;; config
    (setopt citar-org-roam-subdir "hugo/org/literature")
...
#+begin_src emacs-lisp :tangle yes
  ...
  (with-eval-after-load 'copilot
    ...
    (when-let ((path (getenv "COPILOT_LANGUAGE_SERVER_PATH")))
      (setopt copilot-server-executable path))
...
#+begin_src emacs-lisp :tangle yes
  ...
  (with-eval-after-load 'copilot-chat
    ...
    (setopt copilot-chat-default-model "claude-sonnet-5"))
...
:PROPERTIES:
:CUSTOM_ID: my-beginning-of-intendation
:END:
#+begin_src emacs-lisp :tangle yes
...
:PROPERTIES:
:CUSTOM_ID: my-reload-major-mode
:END:
#+begin_src emacs-lisp :tangle yes
...
:PROPERTIES:
:CUSTOM_ID: my-toggle-kill-emacs
:END:
#+begin_src emacs-lisp :tangle yes
  ...
  (defun my/today (&optional iso-format)
    ...
    (insert
     (format-time-string
      ...
      (current-time))))
#+end_src
** my/delete-forward-block
:PROPERTIES:
:CUSTOM_ID: my-subword
:END:
#+begin_src emacs-lisp :tangle yes
...
:PROPERTIES:
:CUSTOM_ID: my-insert-date-at-point
:END:
#+begin_src emacs-lisp :tangle yes
...
:PROPERTIES:
:CUSTOM_ID: my-copy-projectile-github-url
:END:
#+begin_src emacs-lisp
  (defun my/copy-projectile-github-url ()
    ...
    (let* ((project-root (projectile-project-root))
           ...
           ;; A worktree created under the bare layout is detached, and
           ;; --abbrev-ref then reports the literal string "HEAD", which would
           ;; put GitHub's default branch in the URL instead of the commit
           ;; actually being read.  Name the commit in that case.
           (branch (let ((ref (string-trim (shell-command-to-string "git rev-parse --abbrev-ref HEAD"))))
                     (if (string= ref "HEAD")
...
:PROPERTIES:
:CUSTOM_ID: my-copy-projectile-relative-path
:END:
#+begin_src emacs-lisp
  (defun my/copy-projectile-relative-path (arg)
    "Copy the project-relative path to kill-ring.
With prefix argument ARG, include line number as #L<line>."
    (interactive "P")
    (let* ((relative-path (string-remove-prefix (projectile-project-root) buffer-file-name))
           (result (if arg
                       (format "%s#L%d" relative-path (line-number-at-pos))
                     relative-path)))
      (kill-new result)
      (message "Copied to clipboard: %s" result)))
#+end_src
** my/open-projectile-github-url
#+begin_src emacs-lisp
  (defun my/open-projectile-github-url ()
    "Open the GitHub URL of the current line in the browser."
    (interactive)
    (my/copy-projectile-github-url)
...
:PROPERTIES:
:CUSTOM_ID: enable-magic-file-name
:END:
#+begin_src emacs-lisp :tangle yes
