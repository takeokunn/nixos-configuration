...
:END:
#+begin_src emacs-lisp :tangle yes
  ;; load compat-31 explicitly; it appears after compat-30 in load-path otherwise
...
#+begin_src emacs-lisp :tangle yes
  ...
  ;; daemon evaluates (display-graphic-p) once at start, not per-frame, so hook per-frame instead
...
** gleam-ts-mode
:PROPERTIES:
:CUSTOM_ID: gleam-ts-mode
:END:
#+begin_src emacs-lisp :tangle yes
  (autoload-if-found '(gleam-ts-mode) "gleam-ts-mode" nil t)
...
#+begin_src emacs-lisp :tangle yes
  (define-derived-mode phel-mode clojure-mode "Phel"
    ...
    ;; disable lockfiles: they touch buffer-file-name in a way that confuses the running Phel process during ILT evaluation
...
failing that the newest timestamped one, since those names sort chronologically.
...
independently once created, as a one-shot snapshot rather than something
...
projects: prune step is deliberately NOT reimplemented here because it needs YAML
...
`__fzf_ghq_new_worktree \"\" \"\" branch', using the same base-ref resolution as
...
`projectile-switch-project-action' and existing hooks fire unchanged."
    (interactive)
    ;; :override advice on this symbol discards the :before advice above, so refresh explicitly here
  ...
  (add-hook 'after-init-hook
            #'(lambda ()
                ...
                (when (functionp 'set-fontset-font)
                  ...
                  (set-fontset-font t '(#xfe50 . #xfe6b) "Noto Sans CJK JP")
            ...
            #'(lambda ()
                (when (require 'doom-themes)
                  (let ((theme 'doom-dracula))
                    (load-theme theme t)
                    ;; workaround a mutual face-inheritance cycle between gnus-group-news-low(-empty) in doom-themes;
                    ;; must fix here since gnus-group.el loads lazily and would error before any hook could run
...
#+begin_src emacs-lisp :tangle yes
  ...
  (with-eval-after-load 'doom-modeline
    ...
    ;; doom-modeline-def-segment is a macro; with-eval-after-load doesn't defer compile-time macroexpansion
...
#+begin_src emacs-lisp :tangle yes
  ...
  (add-to-list 'auto-mode-alist (cons (concat (regexp-quote (my/ghq-repo-path "github.com/takeokunn-private/private-pass")) "/current/.*\\.gpg") 'pass-view-mode))
...
#+begin_src emacs-lisp :tangle yes
  ...
  (with-eval-after-load 'copilot-chat-prompts
    (let ((my/copilot-chat-prompt "日本語で回答してください。\nコードと概念を正確に説明し、ユーザーの要件に従ってください。\n名前を尋ねられた場合は「GitHub Copilot」と答えてください。\n専門知識はソフトウェア開発に限定してください。\nMicrosoft のコンテンツポリシーに従い、著作権を侵害する内容は避けてください。\nソフトウェア開発以外の質問には、AIプログラミングアシスタントであることを簡潔に伝えてください。\n回答は簡潔で非個人的な Markdown 形式にしてください。\nコードブロックの先頭に言語名を記述し、回答全体を三重バッククオートで囲まないでください。\n提供されたコード、質問、エラー、プロジェクト情報を確認してから回答してください。\n不明確な点がある場合は確認し、必要に応じて可読性やパフォーマンスの改善案を示してください。\n"))
...
#+begin_src emacs-lisp
  (defun my/copy-projectile-github-url ()
    ...
    (let* ((project-root (projectile-project-root))
           ...
           ;; a detached worktree reports literal "HEAD" here; name the commit instead
